...
Displays pre-collected prospect data from the seed snapshot.
Data is generated locally by running ``python main.py`` and committed
to the repository.  Government APIs are too slow / rate-limited to
call live from a hosted environment.
"""
...
def _load_seed_snapshot():
    """Load the committed seed snapshot.
...
if last_status:
    st.sidebar.markdown("---")
    st.sidebar.caption("Data collection status:")
...
st.sidebar.markdown("---")
st.sidebar.info(
    "Data is pre-collected and committed to the repository. "
    "Run `python main.py` locally to refresh with the latest "
...
if prospects_data is None:
    ...
    st.warning(
        "No seed data found. Run `python main.py` locally to collect "
        "funding data from SAM.gov, USASpending, and SBIR sources, "
        "then commit and push `seed_data/prospect_snapshot.json`."
    )
    st.stop()
...
st.title("A&D Prospect Engine")
st.caption("Data collected: {}".format(collected_at))
